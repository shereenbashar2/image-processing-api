...
describe('Image Processing Service', () => {
  ...
  it('should not throw an error during image resizing', async () => {
    const imagePath = path.resolve(__dirname, '../../../assets/original/encenadaport.jpg');
  ...
  it('should resize a JPEG image successfully', async () => {
    ...
    const imagePath = path.resolve(__dirname, '../../../assets/original/food1.jpeg');

    const processingOptions = { width: 100, height: 100 }; 
    const imageQuality = 80;
    const resizedImageBuffer = await resizeImage(imagePath, processingOptions, ImageFormat.JPEG, imageQuality);
  ...
  it('should resize a PNG image successfully', async () => {
    const imagePath = path.resolve(__dirname, '../../../assets/original/food.png');
    const processingOptions = { width: 200, height: 200 };
  
    const imageQuality = 90;

    const resizedImageBuffer = await resizeImage(imagePath, processingOptions,  ImageFormat.PNG, imageQuality);
